...
use super::font_program::{CharmapType, load_freetype_face, use_charmap};
...
impl SimpleFont {
    ...
    pub fn try_new<R: Seek + Read>(dict: &PdfDict, ctx: &PDFContext<R>) -> PdfResult<Self> {
        let subtype = match dict.get("Subtype") {
            ...
            None => {
                return Err(PdfError::FontError(
                    "Simple font subtype is None".to_string(),
                ));
            }
        };
        ...
        if let Some(bf) = dict.get("BaseFont") {
            let name = bf.as_name().unwrap();
            font.base_font = name.name().to_string();
        }

        if let Some(first_char) = dict.get("FirstChar") {
            font.first_char = first_char.as_u32();
        }

        if let Some(last_char) = dict.get("LastChar") {
            font.last_char = last_char.as_u32();
        }
        font.load_descriptor(dict, ctx)?;
        font.load_encoding(dict, ctx)?;
        font.load_to_unicode(dict, ctx)?;
        font.load_width(dict, ctx)?;
    ...
    fn init_truetype_cid_to_gid(&mut self) -> PdfResult<()> {
    ...
    fn init_type1_cid_to_gid(&mut self) -> PdfResult<()> {
        if let Some(fontfile) = self.fontfile() {
            let face = load_freetype_face(fontfile.to_vec())?;
            for charcode in 0..=255 {
    ...
    fn init_cid_to_gid(&mut self) -> PdfResult<()> {
        match self.subtype {
    ...
    fn load_to_unicode<R: Seek + Read>(
        &mut self,
        dict: &PdfDict,
        ctx: &PDFContext<R>,
    ) -> PdfResult<()> {
        if let Some(tu) = dict.get("ToUnicode") {
            let tuobj = ctx.resolve(tu)?;
            let data = tuobj.as_stream().unwrap().decode_data(ctx)?;
            let cmap = Cmap::try_new(data)?;
            self.to_unicode = Some(cmap);
        }
        Ok(())
    }

    fn load_width<R: Seek + Read>(&mut self, dict: &PdfDict, ctx: &PDFContext<R>) -> PdfResult<()> {
        if let Some(ws) = dict.get("Widths") {
            let ws = ctx.resolve(ws)?;
            let wsa = ws.as_array().unwrap();
            let mut widths = [0.0; 256];
            for (i, v) in wsa.into_iter().enumerate() {
                let index = i + self.first_char.unwrap() as usize;
                widths[index] = v.as_f32().unwrap();
            }
        ...
        if let Some(name) = find_builtin_font(self.base_font()) {
            if let Some(afm_data) = load_builtin_metrics(name) {
                let s = String::from_utf8(afm_data.to_vec())
                    .map_err(|_| PdfError::FontError("afm data not utf8".to_string()))?;
                let afm = parse_afm(s)?;
                self.afm = Some(afm);
            }
        }
    ...
    fn load_descriptor<R: Seek + Read>(
        &mut self,
        dict: &PdfDict,
        ctx: &PDFContext<R>,
    ) -> PdfResult<()> {
        if let Some(desc) = dict.get("FontDescriptor") {
            let desc_dict = ctx.resolve(desc)?.as_dict().unwrap();
            let desc = Descriptor::try_new(desc_dict, ctx)?;
    ...
    fn load_encoding<R: Seek + Read>(
        &mut self,
        dict: &PdfDict,
        ctx: &PDFContext<R>,
    ) -> PdfResult<()> {
        if let Some(enc) = dict.get("Encoding") {
            let enc = ctx.resolve(enc)?;
            match enc {
                PdfObject::PdfDict(d) => {
    ...
    pub fn char_width(&self, code: u8) -> PdfResult<f32> {
        ...
        return Err(PdfError::FontError("Font width is erro".to_string()));
    }
        ...
        match &self.to_unicode {
            ...
            None => match &self.encoding {
                ...
                None => {
                    ...
                    Err(PdfError::FontError(
                        "Simple Font to_unicode is None".to_string(),
                    ))
                }
            },
        }
