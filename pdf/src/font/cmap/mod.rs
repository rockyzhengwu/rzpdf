...
impl Cmap {
    pub fn try_new(data: Vec<u8>) -> PdfResult<Self> {
        let mut parser = CmapParser::new(data);
        parser.parse()
    }

    pub fn new_from_predefined(name: &str) -> PdfResult<Self> {
        let cmap_data = get_predefine_cmap_data(name).ok_or(PdfError::FontError(format!(
