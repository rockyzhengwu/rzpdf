...
pub fn ascii_hex_decode(input: &[u8]) -> PdfResult<Vec<u8>> {
    ...
    for c in input {
        ...
        if hex_buffer.len() == 2 {
            let byte = u8::from_str_radix(&hex_buffer, 16).map_err(|_| {
                PdfError::FilterError(format!("Failed to parse hex pair: {}", hex_buffer))
            })?;
            bytes.push(byte);
            hex_buffer.clear();
        }
    ...
    if !hex_buffer.is_empty() {
        ...
        let byte = u8::from_str_radix(&padded_hex, 16).map_err(|_| {
            PdfError::FilterError(format!("Failed to parse padded hex: {}", padded_hex))
        })?;
        bytes.push(byte);
    }
    Ok(bytes)
