...
use super::fax_table::{Color, Mode, decode_mode, decode_run_length};
...
impl<'a> Group4Decoder<'a> {
    ...
    pub fn decode_line(&mut self) -> PdfResult<Vec<u8>> {
        let mut a0 = 0;
        let mut color = Color::White;
        let mut refstate = RefState::new(self.reference.as_slice());
        loop {
            let mode = decode_mode(&mut self.reader).ok_or(PdfError::FilterError(
                "Ccittfax Mode is not detected".to_string(),
            ))?;
            match mode {
                Mode::Pass => {
                    let _b1 = refstate
                        .next_change(!color, a0)
                ...
                Mode::Horizontal => {
                    let a0a1 = decode_run_length(&mut self.reader, color).ok_or(
                        PdfError::FilterError("Ccittfax runlength decode error".to_string()),
                    )?;
...
pub fn decode_g4(buffer: &[u8], param: &Param) -> PdfResult<Vec<u8>> {
