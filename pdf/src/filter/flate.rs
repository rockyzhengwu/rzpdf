    ...
    if let Some(params) = params {
        if let Some(predictor) = params.get("Predictor") {
            let columns = match params.get("Columns") {
                Some(v) => v.as_u32().ok_or(PdfError::FilterError(
                    "Flated decode Columns  is not a number".to_string(),
                ))? as usize,
                None => 1,
            };
            ...
            let colors = match params.get("Colors") {
                Some(v) => v.as_u32().ok_or(PdfError::FilterError(
                    "Flated docde Colors is not a number".to_string(),
                ))? as usize,
                None => 1,
            };
            ...
            let bits_per_component = match params.get("BitsPerComponent") {
                Some(v) => v.as_u32().ok_or(PdfError::FilterError(
                    "Flated decode error BitsPerComponent is not number".to_string(),
                ))? as usize,
                None => 8,
            };
            ...
            let p = predictor.as_u32().ok_or(PdfError::FilterError(
                "Flate decode Predictor is not a number".to_string(),
            ))?;
            if p >= 10 {
                let mut i = 0;
                    ...
                    let decoded_row = match filter_type {
                        ...
                        _ => {
                            return Err(PdfError::FilterError(format!(
                                "Flated decoder Unknown PNG predictor {}",
                                filter_type
                            )));
                        }
                    };
