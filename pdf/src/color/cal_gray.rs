    ...
    pub fn try_new<R: Seek + Read>(obj: &PdfObject, ctx: &PDFContext<R>) -> PdfResult<Self> {
        let color_dict = match obj {
            PdfObject::PdfArray(array) => {
                ...
                let cd = ctx
                    ...
                    .as_dict()
                    .ok_or(PdfError::ColorError("CalGray need dict".to_string()))?;
                cd
            }
            PdfObject::PdfDict(d) => d,
            _ => {
                return Err(PdfError::ColorError("Bad CalGray Color".to_string()));
            }
        };

        let mut color = CalGray::default();
        if let Some(g) = color_dict.get("Gamma") {
            color.gamma = g.as_f32().ok_or(PdfError::ColorError(
                "Cal Gray gamma is not float".to_string(),
            ))?;
        }
        ...
        match color_dict.get("WhitePoint") {
            Some(wh) => {
                let wha = wh.as_array().ok_or(PdfError::ColorError(format!(
                    "CalGray WhitePoint is not an array got:{:?}",
                    obj
                )))?;
                if wha.len() != 3 {
        ...
        if let Some(bp) = color_dict.get("BlackPoint") {
            let bpa = bp.as_array().ok_or(PdfError::ColorError(format!(
                "CalGray BlackPoint is not an array got:{:?}",
                obj
            )))?;
            if bpa.len() != 3 {
                return Err(PdfError::ColorError(format!(
            ...
            for i in 0..3 {
                let v = bpa.get(i).unwrap().as_f32().unwrap();
